...
T('Granting edit lets a technician send a draft quote',()=>{const b=app();tick(b,'technician','edit',true);const c=app(b.storage(),{anon:true});c.login('marcus.lee','tech123');c.openOrder('WO-1047');c.click('[data-action="start-insp"]');c.checkAll();c.click('[data-action="finish-insp"]');c.tab('items');c.click('[data-action="add-part"]');c.F('sku').value='BRK-PADF1';c.submit();return !!c.$('[data-action="send-quote"]');});
T('Recording the customer answer is its own permission',()=>{const b=app();tick(b,'technician','edit',true);const c=app(b.storage(),{anon:true});c.login('marcus.lee','tech123');c.openOrder('WO-1043');return !c.$('[data-action="approve-quote"]')&&c.$('#panelFoot').textContent.includes('waiting for the customer');});
T('Granting it lets that role approve',()=>{const b=app();tick(b,'technician','approve',true);const c=app(b.storage(),{anon:true});c.login('marcus.lee','tech123');c.openOrder('WO-1043');return !!c.$('[data-action="approve-quote"]')&&!!c.$('[data-action="decline-quote"]');});
T('And approving from that role still reserves the parts',()=>{const b=app();tick(b,'technician','approve',true);const c=app(b.storage(),{anon:true});c.login('marcus.lee','tech123');c.openOrder('WO-1043');c.click('[data-action="approve-quote"]');return c.order('WO-1043').stage==='in-service'&&c.part('BRK-PADF1').reserved>0;});
T('Revoking it from an advisor removes both buttons',()=>{const b=app();tick(b,'advisor','approve',false);const c=app(b.storage(),{anon:true});c.login('priya.nair','advisor123');c.openOrder('WO-1043');return !c.$('[data-action="approve-quote"]')&&!c.$('[data-action="decline-quote"]');});
T('And the action is refused even if the button is forced',()=>{const b=app();tick(b,'advisor','approve',false);const c=app(b.storage(),{anon:true});c.login('priya.nair','advisor123');c.openOrder('WO-1043');const e=c.d.createElement('button');e.dataset.action='approve-quote';c.$('#panelFoot').appendChild(e);c.click(e);return c.order('WO-1043').stage==='quotation'&&c.toast().includes("cannot record");});
...
T('The Money owed chip sits between Awaiting payment and Completed',()=>{const b=app();b.go('orders');const fs=b.$$('#v-orders .chips .chip').map(c=>c.dataset.f);return fs.indexOf('owed')===fs.indexOf('awaiting-payment')+1&&fs.indexOf('completed')===fs.indexOf('owed')+1;});
T('The chip order matches the job progress bar',()=>{const b=app();b.openOrder('WO-1045');b.click('[data-action="take-payment"]');b.set('settled','no');b.submit();const bar=[...b.$$('#panelBody .stepper .step span')].map(e=>e.textContent);b.click('[data-action="close-panel"]');b.go('orders');const chips=b.$$('#v-orders .chips .chip').map(c=>c.textContent.replace(/[0-9]+$/,'').trim());return bar.every(step=>chips.indexOf(step)>-1)&&chips.indexOf('Money owed')<chips.indexOf('Completed');});
